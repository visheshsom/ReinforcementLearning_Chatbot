...
def words_to_words(words, emb_dict, rev_emb_dict, net, device, use_sampling=False):
    tokens = data.encode_words(words, emb_dict)
    input_seq = model.pack_input(tokens, net.emb, device)
...
def process_string(s, emb_dict, rev_emb_dict, net, device, use_sampling=False):
    out_words = words_to_words(
        s, emb_dict, rev_emb_dict, net, device, use_sampling=use_sampling
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Enable self-loop mode with given amount of phrases.",
    )
    parser.add_argument(
        "--cuda", action="store_true", default=False, help="Enable CUDA (use GPU)"
    )
    ...
    while True:
        ...
        for _ in range(args.self):
            words = words_to_words(
                words, emb_dict, rev_emb_dict, net, device, use_sampling=args.sample
            )
