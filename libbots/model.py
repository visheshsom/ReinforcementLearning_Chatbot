...
class PhraseModel(nn.Module):
    def __init__(self, emb_size, dict_size, hid_size):
        ...
        self.decoder = nn.LSTM(
            input_size=emb_size, hidden_size=hid_size, num_layers=5, batch_first=True
        )
        self.output = nn.Sequential(nn.Linear(hid_size, dict_size))
        # Add this line for dropout
    ...
    def decode_one(self, hid, input_x):
        ...
        # Unpack the output if it's packed
        if isinstance(out, rnn_utils.PackedSequence):
...
def pack_input(input_data, embeddings, device="cuda"):
    input_v = torch.LongTensor([input_data]).to(
        device
    )  # Move input to the correct device
    r = embeddings(input_v)  # Apply embeddings
